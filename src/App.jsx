...
const CONFIG = {
  profile: {
    ...
    interests: [
      "AI & Machine Learning","Quantitative Finance","Data Structures & Algorithms",
      "Mathematics","Physics","Data Analytics","Geopolitics","Research & Projects","Cricket"
    ],
    focus: [
      "AI & Machine Learning","Deep Learning","Generative AI",
      "Natural Language Processing","Data Structures & Algorithms","AI/ML Projects","Hackathons"
    ],
  },
  dsa: {
...
const P = {
  ...
  text: "#F5F0E8",
  cards: [
    { accent: "#5C9A5C",  label: "Sage" },
    { accent: "#4A9A8A",  label: "Teal" },
    { accent: "#9A6AAA",  label: "Violet" },
    { accent: "#5A90AA",  label: "Blue" },
    { accent: "#C4A060",  label: "Amber" },
    { accent: "#E8906A",  label: "Orange" },
    { accent: "#5C9A5C",  label: "Forest" },
    { accent: "#C4A060",  label: "Sand" },
  ],
};
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {pct}<span style={{ fontSize: "0.38em", letterSpacing: "-2px", opacity: 0.4 }}>%</span>
      </div>
      <div style={{
        fontFamily: "'Poppins', sans-serif",
        fontSize: 13, letterSpacing: "4px", textTransform: "uppercase",
        color: "rgba(245,240,232,0.25)", marginTop: 32, fontWeight: 400,
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: "rgba(255,255,255,0.05)" }}>
        <div style={{
          ...
          background: "linear-gradient(90deg, #5C9A5C, #C4A060)",
...
function PageHeader({ title, accent, onBack }) {
  return (
    <div style={{ padding: "36px 0 44px", borderBottom: "1px solid rgba(255,255,255,0.07)", marginBottom: 44 }}>
      ...
      <h1 style={{
        fontFamily: "'Poppins', sans-serif",
        fontSize: "clamp(44px, 5vw, 70px)", fontWeight: 600,
        color: "#F5F0E8", margin: 0, letterSpacing: "-1.5px",
        borderLeft: `4px solid ${accent}`, paddingLeft: 22, lineHeight: 1.1,
      }}>{title}</h1>
    </div>
  );
}
...
function CardTitle({ children, accent }) {
  return (
    <h2 style={{
      fontFamily: "'Poppins', sans-serif",
      fontSize: 33, fontWeight: 600, color: "#F5F0E8",
      marginBottom: 32, letterSpacing: "-0.5px",
      borderBottom: `2px solid ${accent}`,
      paddingBottom: 12, display: "inline-block", lineHeight: 1.2,
    }}>{children}</h2>
  );
}
    ...
    <div>
      <div style={{ minHeight: "56vh", display: "flex", flexDirection: "column", justifyContent: "flex-end", paddingBottom: 72, paddingTop: 88 }}>
        <div style={{ opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(20px)", transition: "all 1s ease 0.1s" }}>
          <span style={{
            fontFamily: "'Poppins', sans-serif", fontSize: 11, letterSpacing: "3px",
            textTransform: "uppercase", color: "#5C9A5C",
            background: "rgba(92,154,92,0.12)", border: "1px solid rgba(92,154,92,0.22)",
            padding: "5px 14px", borderRadius: 20, display: "inline-block", marginBottom: 26, fontWeight: 500,
          }}>Progressing · 2025–26</span>
        </div>
        <div style={{ opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(28px)", transition: "all 1s ease 0.2s" }}>
          <h1 style={{
            fontFamily: "'Poppins', sans-serif",
            fontSize: "clamp(64px, 7.5vw, 110px)", fontWeight: 600,
            color: "#F5F0E8", margin: 0, lineHeight: 1.0, letterSpacing: "-3px",
      ...
      <div style={{ height: 1, background: "linear-gradient(to right, transparent, rgba(255,255,255,0.08), transparent)", marginBottom: 60 }} />

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22 }}>
        {NAV_ITEMS.map((item, i) => (
          <Reveal key={item.id} delay={i * 0.06}>
            <NavCard item={item} color={P.cards[item.idx]} navigate={navigate} />
          </Reveal>
        ))}
      </div>
      <div style={{ height: 88 }} />
          ...
          <Card accent={P.cards[2].accent}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 11 }}>
              {profile.interests.map(i => (
                <span key={i} style={{
                  ...
                  border: `1px solid ${P.cards[2].accent}35`,
                }}>{i}</span>
              ))}
            </div>
            <div style={{ marginTop: 26 }}>
              <a href={profile.cv} style={{
                ...
                background: "#F5F0E8", color: "#141414",
...
function StatusBox({ states, value, onChange }) {
  ...
  return (
    <button onClick={cycle} style={{
      ...
      transition: "all 0.18s ease", lineHeight: 1.6,
    }}>{value}</button>
  );
}
      ...
      <Reveal>
        <Card accent={color.accent}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", flexWrap: "wrap", gap: 18, marginBottom: 34 }}>
            ...
            <a href={CONFIG.dsa.striverLink} target="_blank" rel="noreferrer" style={{
              fontFamily: "'Poppins', sans-serif", fontSize: 13, color: color.accent,
              textDecoration: "none", fontWeight: 600,
              borderBottom: `1px solid ${color.accent}`, paddingBottom: 2,
            }}>Striver's Sheet ↗</a>
          </div>
                ...
                <div style={{ width: "100%", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    {[
                      ...
                      { label: "Medium", field: "medium",   states: EMH_STATES, c: "#C4A060" },
                      { label: "Hard",   field: "hard",     states: EMH_STATES, c: "#C88080" },
                      { label: "Rev",    field: "revision", states: REV_STATES, c: "#9090D8" },
...
function AIMLPage({ onBack }) {
  ...
  const getTP = (t) => {
    ...
    return { done, total: t.subtopics.length, pct: Math.round((done / t.subtopics.length) * 100) };
  };
  useEffect(() => {
    function handleClickOutside(event) {
      if (expandedTopic && containerRef.current && !containerRef.current.contains(event.target)) {
              ...
              return (
                <div key={t.name} style={{ position: "relative", overflow: "visible", zIndex: isOpen ? 50 : 1 }}>
                  <button onClick={() => toggle(t.name)} style={{
                    ...
                    cursor: "pointer", padding: "16px 20px", textAlign: "left", borderRadius: 15,
                  }}>
                  ...
                  {isOpen && (
                    <div style={{
                      ...
                      padding: "0 20px 16px", borderRadius: 16,
                      ...
                      {t.subtopics.map(s => {
                        ...
                        return (
                          <label key={s} style={{ display: "flex", alignItems: "center", gap: 11, padding: "7px 0", cursor: "pointer" }}>
                            <div onClick={() => toggleSub(t.name, s)} style={{
                              width: 17, height: 17, borderRadius: 5, flexShrink: 0,
                              background: isDone ? color.accent : "transparent",
                              border: `1.5px solid ${isDone ? color.accent : "rgba(255,255,255,0.2)"}`,
            ...
            {currentTopic.videos.map(v => {
              ...
              return (
                <label key={v.id} style={{
                  ...
                  background: done ? `${color.accent}18` : "rgba(255,255,255,0.04)",
                  borderRadius: 11, padding: "11px 13px", cursor: "pointer",
                  border: done ? `1.5px solid ${color.accent}40` : "1px solid rgba(255,255,255,0.06)",
                  transition: "all 0.2s",
                }}>
                  <div onClick={() => toggle(v.id)} style={{
                    ...
                    border: `1.5px solid ${done ? color.accent : "rgba(255,255,255,0.2)"}`,
                    display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s",
                  }}>
...
function CalendarPage({ onBack }) {
  ...
  const pastMonths = allMonths.filter(({ year, month }) =>
  ...
  const getEff = (y, m) => {
    ...
    let ticks = 0;
    for (let d = 1; d <= days; d++) { if (marks[`${y}-${m}-${d}`] === "tick") ticks++; }
  ...
  const getBestStreak = () => {
    ...
    for (const { year, month } of allMonths) {
      ...
      for (let d = 1; d <= days; d++) {
        if (marks[`${year}-${month}-${d}`] === "tick") { current++; best = Math.max(best, current); }
        else { current = 0; }
      }
    }
    return best;
  ...
  const getBestMonth = () => {
    ...
    for (const { year, month } of allMonths) {
      ...
      if (pct > best.pct) best = { label: `${monthNamesShort[month]} ${year}`, pct };
    }
    return best;
  };
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) setVisiblePastCount(prev => Math.min(prev + 1, pastMonths.length));
...
WebkitBackdropFilter: "blur(24px)",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <span style={{
              ...
              background: eff >= 70 ? "rgba(74,154,74,0.15)" : eff >= 40 ? `${color.accent}18` : "rgba(232,144,106,0.15)",
              padding: "4px 14px", borderRadius: 20,
              border: `1px solid ${eff >= 70 ? "rgba(74,154,74,0.3)" : eff >= 40 ? `${color.accent}35` : "rgba(232,144,106,0.3)"}`,
            }}>{eff}% efficient</span>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 5 }}>
          {Array.from({ length: firstDay }, (_, i) => <div key={`e${i}`} style={{ minHeight: isPast ? 68 : 82 }} />)}
          {Array.from({ length: days }, (_, i) => {
            ...
            return (
              <div key={day} style={{
                ...
                display: "flex", flexDirection: "column", padding: isPast ? "7px 9px" : "8px 10px",
                opacity: isFuture ? 0.25 : 1, transition: "background 0.15s, border 0.15s",
                cursor: !isFuture ? "pointer" : "default",
              }} onClick={() => !isFuture && toggleDay(key)}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4, flexShrink: 0 }}>
                  <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 13, fontWeight: mark ? 700 : isToday ? 600 : 400, color: numColor, lineHeight: 1, userSelect: "none" }}>{day}</span>
                  <span style={{
                    display: "inline-flex", alignItems: "center", justifyContent: "center",
                    width: 20, height: 20, borderRadius: "50%", fontSize: 11, fontWeight: 700, flexShrink: 0, userSelect: "none",
                    background: mark === "tick" ? "#4A7A4A" : mark === "cross" ? "#C4714A" : "transparent",
                    border: mark === "tick" ? "2px solid #4A7A4A" : mark === "cross" ? "2px solid #C4714A" : "1.5px solid rgba(255,255,255,0.15)",
                    color: mark ? "#FFF" : "rgba(255,255,255,0.2)",
                  }}>{mark === "tick" ? "✓" : mark === "cross" ? "✗" : "·"}</span>
                </div>
                {!isFuture && (
                  <textarea
                    rows={isPast ? 1 : 2} maxLength={60} placeholder="note…" value={noteVal}
    ...
    <div style={{ width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18, marginBottom: 28 }}>
          {[
            ...
            <div key={s.label} style={{
              ...glass, borderRadius: 18, padding: "24px 26px",
              background: s.highlight ? `${color.accent}12` : "rgba(255,255,255,0.05)",
              border: s.highlight ? `1.5px solid ${color.accent}40` : "1px solid rgba(255,255,255,0.07)",
      ...
      {pastMonths.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
          ...
          <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 11, color: "rgba(245,240,232,0.22)", letterSpacing: "2px", textTransform: "uppercase", fontWeight: 500 }}>Past months</span>
          <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.07)" }} />
        </div>
      )}
...
function AcademicsPage({ onBack }) {
  ...
  const creditsByCourse = { "CS301 - Algorithms": 4, "CS302 - OS": 4, "CS303 - DBMS": 4, "MA201 - Probability": 3, "CS304 - Computer Networks": 4 };
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {currentSem.courses.map(c => (
                  <div key={c} style={{ display: "grid", gridTemplateColumns: "1fr 90px", gap: 12, alignItems: "center", padding: "13px 14px", background: "rgba(255,255,255,0.05)", borderRadius: 12, borderLeft: `3px solid ${color.accent}` }}>
                    <div style={{ fontFamily: "'Poppins', sans-serif", fontSize: 13, color: "rgba(245,240,232,0.7)", fontWeight: 400 }}>{c}</div>
                    <div style={{ fontFamily: "'Poppins', sans-serif", fontSize: 13, color: "#F5F0E8", fontWeight: 600, textAlign: "right" }}>{creditsByCourse[c] ?? "-"}</div>
                  </div>
                ))}
              </div>
            </div>
...
// ─── PROJECTS PAGE ────────────────────────────────────────────────────────────
const STATUS_COLORS = {
  "Completed":   { bg: "rgba(74,154,74,0.18)",   text: "#6ABB7A" },
  "In Progress": { bg: "rgba(74,90,154,0.18)",   text: "#8AAAE8" },
  "Planned":     { bg: "rgba(196,160,96,0.18)",  text: "#C4A060" },
  "On Hold":     { bg: "rgba(232,144,106,0.18)", text: "#E8906A" },
};
              ...
              return (
                <div key={p.name} style={{ background: "rgba(255,255,255,0.05)", borderRadius: 18, padding: "24px 26px", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                    <h3 style={{ fontFamily: "'Poppins', sans-serif", fontSize: 17, fontWeight: 600, color: "#F5F0E8", margin: 0, lineHeight: 1.35, maxWidth: "68%", letterSpacing: "-0.2px" }}>{p.name}</h3>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {items.map(item => (
            <a key={item.title} href={item.link} target="_blank" rel="noreferrer" style={{
              ...
              padding: "15px 20px", background: "rgba(255,255,255,0.04)", borderRadius: 13,
              textDecoration: "none", border: "1px solid rgba(255,255,255,0.06)", transition: "background 0.2s",
            }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.09)"}
              onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.04)"}
            >
              <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 14, color: "rgba(245,240,232,0.8)", fontWeight: 500 }}>{item.title}</span>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <button onClick={() => navigate("home")} style={{
                  background: "none", border: "none", cursor: "pointer",
                  fontFamily: "'Poppins', sans-serif",
                  fontSize: 18, fontWeight: 600, color: "#F5F0E8", letterSpacing: "-0.5px",
                }}>
                  {CONFIG.profile.name}
                </button>
